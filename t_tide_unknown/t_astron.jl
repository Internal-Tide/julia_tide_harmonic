...
```
compute days from 1 Jan 1 to thetime
```

function date2num(thetime)
    gregorian_time = DateTime(1, 1, 1)
    num_time = ((thetime - gregorian_time).value / 86400000) + 1
    num_time
end


function t_astron(jd)
    ...
    d = date2num(jd) - date2num(DateTime(1899, 12, 31, 12, 0, 0))
    D = d / 10000
    # Compute astronomical constants at time d1.
    args = [1; d; D * D; D^3]

    # These are the coefficients of the formulas in the Explan. Suppl.
    sc = [270.434164; 13.1763965268; -8.5e-05; 3.9e-08]
    hc = [279.696678; 0.9856473354; 2.267e-05; 0.0]
    pc = [334.329556; 0.1114040803; -0.0007739; -2.6e-07]
    npc = [-259.183275; 0.0529539222; -0.0001557; -5e-08]
    # first coeff was 281.220833 in Foreman but Expl. Suppl. has 44.
    ppc = [281.220844; 4.70684e-05; 3.39e-05; 7e-08]
    coef = hcat(sc, hc, pc, npc, ppc)

    # Compute the parameters;
    # we only need the factional part of the cycle.
    astro = mod.(coef' * args / 360, 1)
    ...
    tau = (mod(date2num(jd) + 1, 1) + astro[2] - astro[1])
    astro = vcat(tau, astro)

    # Compute rates of change.
    dargs = [0; 1; 0.0002 * D; 0.0003 * D * D]
